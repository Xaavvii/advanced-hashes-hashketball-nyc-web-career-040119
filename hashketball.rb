...
def team_names
  game_hash.collect do |location, rosters|
    rosters[:team_name]
  end
end

def player_numbers(team_name = "Charlotte Hornets")
  if game_hash[:home][:team_name].include?(team_name)
    game_hash[:home][:players].collect do |nba_star, stats|
      stats[:number]
    end
  elsif game_hash[:away][:team_name].include?(team_name)
    game_hash[:away][:players].collect do |nba_star, stats|
      stats[:number]
    end
  end
end
...
def big_shoe_rebounds
  i = 0
  shoe_array = game_hash.collect do |location, rosters|
    rosters[:players].collect do |nba_star, stats|
      stats[:shoe]
    end
  end.flatten
  rebounds_array = game_hash.collect do |location, rosters|
    rosters[:players].collect do |nba_star, stats|
      stats[:rebounds]
    end
  end.flatten
  ...
  player_rebounds = rebounds_array[0]

  while i < shoe_array.count
    if biggest_shoe < shoe_array[i]
      biggest_shoe = shoe_array[i]
      player_rebounds = rebounds_array[i]
    end
